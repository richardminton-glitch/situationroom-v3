...
function renderTree(treeData, savedTransform = null) {
  ...
  addFilters(defs);
  addArrow(defs, 'arrow-primary', '#ffffff',                1.0, 22, 16); // thick ribbon
  addArrow(defs, 'arrow-ghost',   'rgba(255,255,255,0.3)',  1.0, 13, 10); // medium ribbon
  addArrow(defs, 'arrow-output',  'rgba(255,255,255,0.45)', 1.0,  9,  7); // slim ribbon
...
// ─── SVG filter defs: pure white glow only ──────────────────────────────────
function addFilters(defs) {
  const f = defs.append('filter').attr('id', 'glow-white')
    .attr('x', '-40%').attr('y', '-40%')

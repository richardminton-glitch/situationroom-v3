...
/**
 * /tools/power-law — Bitcoin Power Law (Members + General tier)
 *
 * The canonical Santostasi log-log model. BTC plotted against
 * days-since-genesis on log-log axes, with median fit and support/
 * resistance bands. A second chart shows channel position over time.
 */

import { useAuth } from '@/components/layout/AuthProvider';
import { useTier } from '@/hooks/useTier';
import { UpgradePrompt } from '@/components/auth/UpgradePrompt';
import { PowerLawRoom } from '@/components/power-law/PowerLawRoom';
    ...
    return (
      <div style={{ ...fullScreenStyle, flexDirection: 'column', padding: '40px 20px' }}>
        ...
        <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 24, lineHeight: 1.6, textAlign: 'center', maxWidth: 460 }}>
          The Santostasi log-log model — Bitcoin price as a power
          function of time since genesis. Sign in to view.
        </p>
        <a
          href="/login"
          style={{
  ...
  if (!canAccess('general')) {
    return (
      <div style={{ ...fullScreenStyle, padding: '40px 20px' }}>
        <UpgradePrompt requiredTier="general" featureName="Power Law" variant="overlay" />
      </div>
    );
  }

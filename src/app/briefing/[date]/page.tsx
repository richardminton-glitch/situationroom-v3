...
function SectionBody({ content }: { content: string }) {
  ...
  if (paragraphs.length === 0) {
    return (
      <p style={{ fontFamily: 'var(--font-body)', fontSize: '15px', lineHeight: 1.75, color: 'var(--text-primary)' }}>
        {content}
      </p>
    );
  }
...
export default async function BriefingPage({ params }: Props) {
  ...
  const sections = [
    ...
    { key: 'geopolitical',  title: 'III. Geopolitical Watch',content: briefing.geopoliticalSection },
  ...
  const generatedDate = new Date(briefing.generatedAt).toLocaleTimeString('en-GB', {
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <article className="max-w-2xl mx-auto px-6 py-12">
        ...
        >
          ...
          <span>Situation Room — Daily Briefing</span>
          <span>Generated {generatedDate}</span>
        </div>

        {/* Double rule */}
        <div style={{ borderTop: '3px double var(--border-primary)', marginBottom: '4px' }} />
        <div style={{ borderTop: '1px solid var(--border-primary)', marginBottom: '16px' }} />
            ...
            >
              {[
                ...
                >
                  ...
                  <span style={{ color: 'var(--text-primary)', tabularNums: true } as React.CSSProperties}>
                    {value}
                  </span>
                </div>
              ))}
              ...
              {headlines.slice(0, 24).map((h, i) => (
                ...
                >
                  ...
                  >
                    {h.title}
                    {h.source && (
                      <span style={{ color: 'var(--text-muted)', marginLeft: '6px' }}>— {h.source}</span>
                    )}
                  </span>
                </li>
              ))}

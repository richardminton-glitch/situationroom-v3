...
type RatesData = Record<string, DataPoint[]>;

const KEYS = ['Fed', 'ECB', 'BOE', 'BOJ'] as const;
...
export function CentralBankRatesPanel() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/api/data/cbrates');
  ...
  const render = useCallback(() => {
    ...
    el.querySelectorAll('.cbrates-tooltip').forEach((n) => n.remove());
    ...
    const rawMax = d3.max(allPoints, (d) => d.value) ?? 6;
    const yFloor = Math.min(0, rawMin - 0.1);
    const yCeil  = rawMax * 1.08;
    ...
    // Zero baseline (ECB/BOJ went negative)
    if (yFloor < 0) {
      g.append('line')
        .attr('x1', 0).attr('x2', innerW)
    ...
    const lineGen = d3.line<DataPoint>()
      .x((d) => xScale(d.time))
      .y((d) => yScale(d.value))
      .curve(d3.curveStepAfter); // step curve — rates change discretely at meeting dates

    const areaGen = d3.area<DataPoint>()
      .x((d) => xScale(d.time))
      .y0(yScale(Math.max(0, yFloor)))
      .y1((d) => yScale(d.value))
    ...
    tooltip.className = 'cbrates-tooltip';
    ...
    svg.on('mousemove', function (event) {
      ...
      tooltip.innerHTML = [
        ...
        ...vals.map(({ key, point }) =>
          ...
          `<span style="font-weight:600">${fmtRate(point.value)}</span>`
        ),
      ].join('<br>');

      const elRect = el.getBoundingClientRect();
      const svgRect = (svg.node() as SVGSVGElement).getBoundingClientRect();
      const absX = event.clientX - elRect.left;
      const absY = svgRect.top - elRect.top + MARGIN.top + 10;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (containerRef.current) {
        ...
        containerRef.current.querySelectorAll('.cbrates-tooltip').forEach((n) => n.remove());
      }
    };
  }, [render, resizeKey]);

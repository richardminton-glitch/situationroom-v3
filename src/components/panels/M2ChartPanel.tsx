...
export function M2ChartPanel() {
  ...
  const render = useCallback(() => {
    ...
    activeSeries.forEach(({ key }, i) => {
      ...
      lg.append('line').attr('x1', 0).attr('x2', 14).attr('y1', 0).attr('y2', 0).attr('stroke', color).attr('stroke-width', 2);
      lg.append('text').attr('x', 18).attr('y', 3).attr('fill', tickColor).attr('font-size', '11px').attr('font-family', fontData).text(LABELS[key] ?? key);
    });

    // X-axis
    const xTicks = Math.max(3, Math.floor(innerW / 80));
    g.append('g').attr('transform', `translate(0,${innerH})`)
      .call(d3.axisBottom(xScale).ticks(xTicks).tickSize(0).tickFormat((d) => d3.timeFormat('%b %y')(d as Date)))
    ...
    svg.on('mousemove', function (event) {
      ...
      if (mx < 0 || mx > innerW) { crosshair.style('display', 'none'); tooltip.style.display = 'none'; return; }

...
export interface VipBriefingEmailProps {
  // Briefing content
  ...
  sections: {
    ...
    geo: string;
    macro: string;
    outlook: string;        // personalised outlook for this user's topics
  };
  ...
  // VIP-specific
  topicNames: string[];     // e.g. ['Bitcoin Network', 'On-Chain Analytics']
  poolStatus?: {
    balanceSats: number;
      ...
      <Body style={{ backgroundColor: C.bg, margin: '0', padding: '0', fontFamily: font.serif }}>
        <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '20px 0' }}>
          ...
          <Section style={{ backgroundColor: C.card, borderLeft: `1px solid ${C.border}`, borderRight: `1px solid ${C.border}`, padding: '0 28px 0' }}>
            ...
            {[
              [btcPrice, btcChange24h, fearGreed],
              [hashrate, mvrv, blockHeight],
              [sp500, vix, gold],
              [dxy, us10y, oil],
            ].map((vals, ri) => {
              const labels =
                ri === 0 ? ['BTC PRICE', '24H CHANGE', 'FEAR & GREED'] :
                ri === 1 ? ['HASHRATE', 'MVRV', 'BLOCK HEIGHT'] :
                ri === 2 ? ['S&P 500', 'VIX', 'GOLD'] :
                           ['DXY', 'US 10Y', 'OIL'];
              return (
          ...
          {/* Focus topics bar — VIP only */}
          {topicNames.length > 0 && (
            <Section style={{ backgroundColor: C.card, borderLeft: `1px solid ${C.border}`, borderRight: `1px solid ${C.border}`, padding: '10px 28px' }}>
              <Text style={{ fontFamily: font.mono, fontSize: '9px', letterSpacing: '0.18em', color: C.muted, margin: '0 0 4px' }}>
                PERSONALISED FOR
              </Text>
              <Text style={{ fontFamily: font.mono, fontSize: '11px', color: C.accent, margin: '0', letterSpacing: '0.06em' }}>
                {topicNames.join(' · ')}
              </Text>
              <Hr style={{ borderColor: C.dimBorder, margin: '10px 0 0' }} />
            </Section>
          )}
          ...
          {/* Alerts block — Members/VIP, shown when there are recent bot alerts */}
...
export function vipBriefingSubject(date: string, threatLevel: string, topics: string[]): string {
  const topicStr = topics.slice(0, 2).join(' · ');
  return `Situation Room · VIP · ${threatLevel} · ${date}${topicStr ? ` · ${topicStr}` : ''}`;
}
